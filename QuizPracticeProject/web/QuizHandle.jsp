    ...
    <body>
        ...
        <main>
            ...
            <div class="progress rounded-pill">
                <div role="progressbar" aria-valuenow="${processPercentage}" aria-valuemin="0" aria-valuemax="100" style="width: ${progressPercentage}%"
                ...
                <form action="quizhandle?id=${id}&page=${p}" method="POST">
                    <div class="under-header">
                        ...
                        <div class="under-header-right">
                            <c:if test="${attempt.marked eq false}">
                                <span>
                                    <input type="radio" class="mark-question" id="mark-question" name="mark" value="mark" onclick="submit()">    
                                    <label for="mark-question">Mark Question</label>
                                </span>
                            </c:if>
                            <c:if test="${attempt.marked eq true}">
                                <span>
                                    <input type="radio" class="mark-question" name="mark" id="mark-question" value="unmark" onclick="submit()">
                    ...
                    <div class="game-quiz-container">
                        ...
                        <div class="game-options-container">
                            <span>
                                <input type="radio" id="option-one" name="option" class="radio" value="${question.optionA}" onclick="submit();" <c:if test="${attempt.userAnswer eq question.optionA}">checked</c:if>/>
                                <label for="option-one" class="option" id="option-one-label">A. ${question.optionA}</label>
                            </span>


                            <span>
                                <input type="radio" id="option-two" name="option" class="radio" value="${question.optionB}" onclick="submit();" <c:if test="${attempt.userAnswer eq question.optionB}">checked</c:if>/>
                                <label for="option-two" class="option" id="option-two-label">B. ${question.optionB}</label>
                            </span>


                            <span>
                                <input type="radio" id="option-three" name="option" class="radio" value="${question.optionC}"  onclick="submit();" <c:if test="${attempt.userAnswer eq question.optionC}">checked</c:if>/>
                                <label for="option-three" class="option" id="option-three-label">C. ${question.optionC}</label>
                            </span>


                            <span>
                                <input type="radio" id="option-four" name="option" class="radio" value="${question.optionD}" onclick="submit();" <c:if test="${attempt.userAnswer eq question.optionD}">checked</c:if>/>
