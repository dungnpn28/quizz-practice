        ...
        try {
            for (int rowIndex = 1; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
                ...
                if (row != null) {
                    ...
                    Cell dimensionIdCell = row.getCell(11);
                    if (subjectIdCell != null) {
                        ...
                        int statusC = (int) statusCell.getNumericCellValue();
                        boolean status = false;
                        if (statusC == 1) {
                            status = true;
                        }
                        int dimensionId = (int) dimensionIdCell.getNumericCellValue();
                        Question q = new Question(subjectId, lessonId, content, optionA, optionB, optionC, optionD, answer, answerExplanation, level, status);
                        qe.addQuestion(q);
                        int quesId = qe.getMaxId();
                        qe.addQuestionDimension(quesId, dimensionId);
